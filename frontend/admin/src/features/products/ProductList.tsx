...
export const ProductList: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () => productService.getProducts({ search, page, limit: 10 }),
  });
                  ...
                  <tr key={product.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-6 py-4 pl-10">
                      <div className="flex items-center gap-4">
                        <div className="relative w-14 h-14 rounded-xl border border-slate-200 bg-white flex items-center justify-center overflow-hidden flex-shrink-0 group-hover:scale-105 transition-transform duration-300 shadow-sm">
                           {product.assets?.[0]?.url ? (
                             <img src={product.assets[0].url} alt={product.name} className="w-full h-full object-cover" />
                           ) : (
                             <ImageIcon className="w-6 h-6 text-slate-300" />
                           )}
                           <div className="absolute bottom-1 right-1 bg-slate-900/40 text-[8px] text-white px-1 rounded flex items-center gap-0.5 backdrop-blur-sm">
                             <ImageIcon className="w-2 h-2" /> {product.assets?.length || 0}
                           </div>
                        </div>
                        <div className="flex flex-col min-w-0">
                          <span className="font-extrabold text-slate-800 line-clamp-1 group-hover:text-primary transition-colors text-base">{product.name}</span>
                    ...
                    <td className="px-6 py-4">
                       <span className="font-black text-slate-900 tracking-tight text-base italic">
                    ...
                    <td className="px-6 py-4">
                       <div className="flex items-center justify-center gap-2">
                          ...
                          <Button variant="ghost" className="p-2 h-10 w-10 text-slate-400 hover:bg-slate-100 rounded-full shadow-none border-none transition-all">
