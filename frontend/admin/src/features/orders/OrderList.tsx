  ...
  const getStatusBadge = (status: OrderStatus) => {
    switch (status) {
      case OrderStatus.PENDING:
        return <Badge variant="warning"><Clock className="w-3 h-3" /> Chờ xử lý</Badge>;
      case OrderStatus.PROCESSING:
        return <Badge variant="info"><RefreshCcw className="w-3 h-3" /> Đang chuẩn bị</Badge>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        {[
          ...
          { label: 'Chờ xử lý', value: OrderStatus.PENDING, icon: Clock, color: 'orange' },
          { label: 'Đang chuẩn bị', value: OrderStatus.PROCESSING, icon: RefreshCcw, color: 'blue' },
                  ...
                  <tr key={order.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-5">
                      <div className="flex flex-col">
                        <span className="font-black text-slate-900 group-hover:text-primary transition-colors">#{order.orderNumber}</span>
                        <span className="text-[10px] font-bold text-primary bg-primary/5 w-fit px-1.5 rounded mt-1">{order.items.length} món</span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-400 font-black text-xs uppercase">
                          {order.user?.fullName?.[0] || 'G'}
                        </div>
                        <div className="flex flex-col">
                          <span className="font-extrabold text-slate-800 line-clamp-1">{order.user?.fullName}</span>
                          <span className="text-[11px] font-bold text-slate-400 uppercase tracking-tighter">{order.phone}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[11px] font-bold text-slate-400">{new Date(order.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    </td>
                    <td className="px-8 py-5">
                    ...
                    <td className="px-8 py-5">
                      <div className="flex items-center justify-end gap-2">
                        <select
                          className="bg-slate-100 text-[10px] font-black uppercase px-2 py-1 rounded-lg border-none outline-none cursor-pointer hover:bg-slate-200 transition-colors"
                          value={order.status}
